...
class Car:
    def __init__(self, road, color, speed=2):
        self.road = road
    ...
    def check_turn(self, roads):
        for road in roads:
            if road != self.road:
                ...
                if (x2 - x1) * (y3 - y1) == (y2 - y1) * (x3 - x1) and self.direction == road.name:
                    self.__init__(road, color=self.color)
                    self.direction = None
                    self.x, self.y = x3, y3

    def update(self, cars, roads):
        self.check_turn(roads)
        if self.can_move(cars):
            self.move()
